            ...
            visitor: {
                ...
                FunctionDeclaration(pathNode, state) {
                    ...
                    if (isExportedFunctionPath(pathNode)) {
                        const trackedScriptMetadata = createTrackedScriptMetadata(state, pathNode.node.id.name);
                        if (trackedScriptMetadata) {
                            wrapTrackedScriptEntry(pathNode, state, trackedScriptMetadata);
                        }
                    }
                },
                ...
                VariableDeclarator(pathNode, state) {
                    ...
                    if (isExportedFunctionPath(pathNode)) {
                        const trackedScriptMetadata = createTrackedScriptMetadata(state, pathNode.node.id.name);
                        if (trackedScriptMetadata) {
                            wrapTrackedScriptEntry(initPath, state, trackedScriptMetadata);
                        }
                    }
                },
