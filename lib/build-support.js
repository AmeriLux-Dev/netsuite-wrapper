...
// N/log is the one wrapper that does something without telemetry: it splits long log details.
const TELEMETRY_OFF_OVERRIDE_MODULES = Object.freeze(['log']);

/**
 * The N modules a build swaps for their wrappers. With telemetry off only N/log is swapped, so a
 * project that turned telemetry off has nothing of the wrapper between it and the rest of NetSuite.
 * An explicit `modules` list in the options or the config file always wins.
 */
function resolveOverrideModules(resolvedOptions) {
    if (resolvedOptions.modules) {
        return resolvedOptions.modules;
    }

    if (!resolvedOptions.telemetryBootstrap) {
        return DEFAULT_OVERRIDE_MODULES.filter((moduleName) => TELEMETRY_OFF_OVERRIDE_MODULES.includes(moduleName));
    }

    return DEFAULT_OVERRIDE_MODULES;
}
